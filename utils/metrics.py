...
"""本模块提供了量化评估指标的函数，包括RankIC、ICIR、IC胜率和分组回测。

包含RankIC、ICIR、IC胜率、分组回测等量化评估指标。
"""
...
def ic_summary(ic_series: pd.Series) -> dict[str, float]:
    ...
    Returns:
        包含以下键的字典:
        - rank_ic_mean: RankIC均值，>0.03算有效。
        - icir: IC信息比率=RankIC均值/标准差，>0.5算稳定。
        - ic_win_rate: IC胜率=IC>0的截面占比，>50%算有效。
    """
...
def group_return(
    ...
    Returns:
        分组回测结果DataFrame，列: group, mean_return, date_count。
    """
    # 每个截面内分组
    df = df.copy()
